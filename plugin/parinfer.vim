...
let s:IndentMode = luaeval('require("parinfer").indentMode')
let s:ParenMode = luaeval('require("parinfer").parenMode')
let s:SmartMode = luaeval('require("parinfer").smartMode')
...
function! s:process_buffer() abort
  ...
  if b:parinfer_last_changedtick != b:changedtick
    ...
    let l:request = { 'commentChar': b:parinfer_comment_char,
                    \ 'cursorX': l:cursor[2],
    ...
    if g:parinfer_mode == 'indent'
        let l:response = s:IndentMode(orig_text, l:request)
    elseif g:parinfer_mode == 'paren'
        let l:response = s:ParenMode(orig_text, l:request)
    elseif g:parinfer_mode == 'smart'
        let l:response = s:SmartMode(orig_text, l:request)
    endif
...
augroup Parinfer
  execute 'autocmd FileType '.join(s:FILETYPES, ',').' call <SID>initialize_buffer()'
augroup END

" Handle the case where parinfer was lazy-loaded (i.e. after &filetype
" has been set, so that FileType could not trigger the autocommand above).
if index(s:FILETYPES, &filetype) >= 0
